...
- **Shared / printable reports lead with AI Visibility**, not the structural AI Readiness score. The share page (`app/share/[slug]/page.tsx`) and PDF route (`app/api/generate-pdf/route.ts`) both compute their hero from `ai_visibility_results` via `getLatestVisibilityScanAnyAge()` in `lib/db.ts` and `computeCitationStats()` / `computeVerdict()` in `lib/visibility-report.ts`. The structural score (`overall_score`) appears below the fold as "Why your score could be even higher" — a supporting section, never the headline. Verdict thresholds from citation rate: ≥80 Strong, 60–79 At Risk, 40–59 Low, <40 Critical: Not Cited. When no scan data exists (pre-scan analyses), both paths fall back to the structural-led layout with an amber banner prompting re-analysis. The on-screen `/results` page already handles this correctly — do not touch it.
...
Sequential — don't start next phase until current is stable. Edit freely.
