...
- **Toast notifications use sonner.** Wrapper is `components/ui/sonner.tsx` and `<Toaster />` is mounted once in `app/layout.tsx`. Use `import { toast } from "sonner"` for new toasts. Do NOT add a second Toaster, and do NOT import from `components/ui/toast.tsx` / `toaster.tsx` / `use-toast.ts` / `hooks/use-toast.ts` — those are dead radix-based code awaiting cleanup.
...
| ~~Share page over-triggers pre-scan fallback banner~~ | `app/share/[slug]/page.tsx` | **FIXED** — `usesFallback` now checks only `!hasScan`. Root cause: `citation_gaps` on the `analyses` row is NULL whenever a re-analysis happens within the 72h visibility-scan cache window (cache hit skips the writeback), or when all Perplexity queries error. The share page renders from `scan.results` directly and never needed `citation_gaps`. |